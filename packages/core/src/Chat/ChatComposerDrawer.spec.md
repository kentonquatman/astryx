...
| Area                    | Contract                                                                                                                                                                                                                    |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Public contract         | Add one optional `collapsedSummary?: ReactNode` slot for caller-composed visual summary content.                                                                                                                            |
...
`collapsedSummary` is the complete custom visual summary, not an addition before or
after the default. The component does not parse, clone, decorate, or supply data to
the node. Builders already own the values they pass as `count` and `label`; when a
custom summary forms a sentence or formats a number, its callsite owns the complete
localized message.
...
| FR8  | When `collapsedSummary` is omitted, the collapsed disclosure MUST preserve the existing neutral Badge containing `count`, followed by the resolved `label`. When provided, the node MUST replace that complete visual summary exactly once. | approved caller-composition boundary and released default                            |
