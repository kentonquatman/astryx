...
lane on screen for a measurement that can be held still.`,...B.parameters?.docs?.description}}},V.parameters={...V.parameters,docs:{...V.parameters?.docs,source:{originalSource:`{
  render: args => {
    const [value, setValue] = useState<SearchableItem[]>([users[0], users[2]]);
    return <Tokenizer {...args} searchSource={userSource} value={value} onChange={items => setValue(items)} />;
  },
...
}`,...V.parameters?.docs?.source},description:{story:"The value is busy; the search is idle. `isLoading` means the field's VALUE\nis resolving or being saved — here two members the app is still confirming\n— so the end-lane Spinner shows and the combobox carries `aria-busy`, while\nthe search source stays fully usable: type to search and pick a result.",...V.parameters?.docs?.description}}},H.parameters={...H.parameters,docs:{...H.parameters?.docs,source:{originalSource:`{
  render: args => {
    const [value, setValue] = useState<SearchableItem[]>([users[0]]);
    return <Tokenizer {...args} searchSource={userSource} value={value}
    // The app accepts the change only once the save settles, so the
    // proposed tokens show optimistically and the field stays busy
    // until then.
    onChange={() => {}} changeAction={async items => {
...
}`,...H.parameters?.docs?.source},description:{story:"`changeAction` runs after `onChange` in a transition. Each add, remove,\nBackspace, and clear-all shows the proposed tokens at once and keeps the\nfield busy — one Spinner, `aria-busy` on the combobox — until the save\nsettles (1200 ms here) and the controlled value has caught up.",...H.parameters?.docs?.description}}},U=`Default.WithPreselected.WithClear.MaxEntries.Required.WithDescription.WithError.WithWarning.WithSuccess.Disabled.WithStartIcon.WithStartIconAndTokens.WithEntriesOnFocus.OverflowInline.OverflowLayer.WithEndContent.Creatable.SizeVariants.CreatableWithSearch.DisabledWithMessage.StatusVariantComparison.Loading.LogicalOrder.RtlEndLane.ValueBusy.TransitionAction`.split(`.`)}))();export{N as Creatable,F as CreatableWithSearch,g as Default,T as Disabled,I as DisabledWithMessage,R as Loading,z as LogicalOrder,y as MaxEntries,k as OverflowInline,A as OverflowLayer,b as Required,B as RtlEndLane,P as SizeVariants,L as StatusVariantComparison,H as TransitionAction,V as ValueBusy,v as WithClear,x as WithDescription,j as WithEndContent,O as WithEntriesOnFocus,S as WithError,_ as WithPreselected,E as WithStartIcon,D as WithStartIconAndTokens,w as WithSuccess,C as WithWarning,U as __namedExportsOrder,h as default};